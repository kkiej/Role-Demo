...
float4 DrawCoreFragment(Varyings input, bool isFrontFace : SV_IsFrontFace) : SV_TARGET
{
    ...
    float3 baseColor = tex2D(_BaseMap, input.uv).rgb;
    float4 areaMap = 0;
#if _AREA_FACE
...
#if _AREA_HAIR || _AREA_UPPERBODY || _AREA_LOWERBODY
{
    ...
    int rawIndex = (round((lightMap.a + 0.0425) / 0.0625) - 1) * 0.5;
...
#if _AREA_HAIR || _AREA_UPPERBODY || _AREA_LOWERBODY
{
    ...
    float blinnPhong = pow(saturate(NoH), _SpecularExponent);
...
#if _AREA_UPPERBODY || _AREA_LOWERBODY
{
    ...
    rimLightColor *= _RimLightTintColor.rgb * _RimLightBrightness;
...
#if _EMISSION_ON
{
    ...
    emissionColor *= _EmissionTintColor * _EmissionIntensity;
}
#endif
